...
//	    Elements allJsLinks = doc.select("a[href^='https'],[class*='img_div news_list_img']");	//OR
	    ...
	    for (int i=0;i<allJsLinks.size();i++) {
	    	...
	    	System.out.println(urlNews);//連結網站
	    	System.out.println(pstr);//內容
	        System.out.println(datastr);//日期
	        
	        
	        
	        page = webClient.getPage(urlNews);//從連結網站再往裡面找一次要的資料
	        ...
	        Elements contextarea =docNews.select("[class*='detail_content']").select("p");//活動時間
	        Elements imgarea =docNews.select("[class*='con_img']").select("img");//因為要找圖片所以要爬兩層
	      
	        
	        /*text(),html(),attr()這幾個是比較常用的,很實用的屬性,首先text是獲取該物件的文字資訊,
	         html是獲取該物件下面所有的html資訊(包括文字,有時候有奇效,例如table
	                       我可以將table直接顯示到自己的網頁,還不需要自己寫樣式 .attr()是獲取改物件標籤中某屬性的值*/
	  
	        
	        String textvalue=alltext.text();//h2裡面的文字
	        ...
	        System.out.println("分頁開始");
	        System.out.println(textvalue);//文章標題
	        System.out.println(acttext);//活動時間
	        System.out.println(contextvalue);//文章內容
	        System.out.println(imgpicture);//圖片網址
	        System.out.println("分頁結束");
